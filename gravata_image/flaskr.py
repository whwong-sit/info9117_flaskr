...
@app.route('/')
def show_entries():
    cur = g.db.execute('select title, text, username, email from entries order by id desc')
...
def add_entry():
    if not session.get('logged_in'):
        abort(401)
    g.db.execute('insert into entries (title, text, username, email) values (?,?,?,?)',
                 [request.form['title'], request.form['text'], session['username'],  request.form['email']])
    g.db.commit()
