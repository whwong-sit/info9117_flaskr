...
def show_entries():
    ...
    """
    cur = g.db.execute('select entries.title, entries.text, userPassword.username, entries.start_time, entries.end_time, userPassword.gravataremail, entries.id from entries inner join userPassword on entries.username=userPassword.username order by entries.id desc')
    entries = [dict(title=row[0], text=row[1], username=row[2], start_time=row[3], end_time=row[4], gravataremail=row[5], avimg=avatar(row[5]), id=row[6]) for row in cur.fetchall()]
    return render_template('show_entries.html', entries=entries, entry_id=id)
...
def add_entry():
    ...
    if not session.get('logged_in'):
        abort(401)
    g.db.execute('insert into entries (title, text, username) values (?,?,?)', [request.form['title'], request.form['text'], session['username']])
    g.db.commit()
